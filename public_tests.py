...
def test_tower(target):
    ...
    for layer in target.layers:
        ...
        # Does not work on my notebook because the layer.output.shape's are tuples instead of tensors
        ...
        # Added this to cover the above test
        assert layer.output.shape == tuple(expected[i][1]), \
            f"Wrong number of units in layer {i}. Expected {expected[i][1]} but got {layer.output.shape.as_list()}"
